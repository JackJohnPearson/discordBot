...
const err1 = "I am blocked" // reaction blocked
...
client.on('guildMemberAdd', member => {
    ...
    /*
    if(member.guild.channels.cache.get(backupChannelId)){
        chan = member.guild.channels.cache.get(backupChannelId)
    }
    */
...
///////////////////////////////////////////////////////////
    ...
    if(msg.includes('koa yakuza')||msg.includes('koa touhou')) {
        ...
        if(msg.includes('koa yakuza')){
            if(msg === 'koa yakuza nishiki') resource = createAudioResource('C:\\PROJEKTY\\Bot Creation\\tenyears.mp3');
            else if(msg === 'koa yakuza 4') resource = createAudioResource('C:\\PROJEKTY\\Bot Creation\\forfaith.mp3');
            else if(msg === 'koa yakuza receive1') resource = createAudioResource('C:\\PROJEKTY\\Bot Creation\\recieve1.mp3');
            else {
        ...
        if(msg.includes('koa touhou')){
            if(msg === 'koa touhou eosd st4') { resource = createAudioResource('C:\\PROJEKTY\\Bot Creation\\voile.mp3');
            message.react('❤️').catch(err1 => { message.channel.send('Ktoś mnie blokuje 😢') })}
            else if (msg === 'koa touhou ufo byakuren') resource = createAudioResource('C:\\PROJEKTY\\Bot Creation\\byakuren.mp3');
            else if (msg === 'koa touhou sa satori') resource = createAudioResource('C:\\PROJEKTY\\Bot Creation\\satori.mp3');
            else {
...
/////////////////////////////////////////////////////////////
    ...
    if(msg === 'koa' || msg.includes('<@1085650469860409455>')) {
        message.channel.send('Witam z powrotem')
      return
    }
    if(msg === 'koa dm') {
